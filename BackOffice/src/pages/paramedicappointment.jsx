...
const DoctorCalendar = () => {
  ...
  const [paramedicId, setDoctorId] = useState("");
  ...
  const getDoctorColor = (() => {
    ...
    let colorIndex = 0;
    return (paramedicId) => {
      if (!colorMap.has(paramedicId)) {
        colorMap.set(paramedicId, doctorColors[colorIndex % doctorColors.length]);
        colorIndex++;
      }
      return colorMap.get(paramedicId);
    };
  })();

  useEffect(() => {
    fetch("http://localhost:5000/paramedics/paramedics")
      .then((res) => res.json())
  ...
  const fetchAppointments = async () => {
    try {
      const res = await fetch("http://localhost:5000/appointmentsparamedic");
      const data = await res.json();
       console.log(data)
      const appointmentEvents = data.map((appt) => ({
        title: `${appt.paramedicId?.user?.firstName || "Médecin"} (${appt.paramedicId?.speciality || ""})`,
        start: new Date(appt.start),
        end: new Date(appt.end),
        allDay: false,
        paramedicId: appt.paramedicId?._id,
        id: appt._id,
      }));
  ...
  const filteredEvents = selectedDoctor
    ? events.filter((event) => event.paramedicId === selectedDoctor)
    : events;
...
console.log(availableDoctors)
      for (const doc of doctors) {
        try {
          const response = await fetch("http://localhost:5000/appointmentsparamedic", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ paramedicId: doc._id, start, end }),
          });
  ...
  const handleManualAssignment = async () => {
    if (!startTime || !endTime || !paramedicId) {
    ...
    try {
      const response = await fetch("http://localhost:5000/appointmentsparamedic", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ paramedicId, start, end }),
      });

      const saved = await response.json();
      const doctor = doctors.find((doc) => doc._id === paramedicId);
  ...
  const eventStyleGetter = (event) => {
    const backgroundColor = getDoctorColor(event.paramedicId);
    return {
      style: {
        backgroundColor,
        color: "white",
