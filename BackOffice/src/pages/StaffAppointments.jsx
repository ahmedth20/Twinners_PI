...
const DoctorCalendar = () => {
  ...
  const [staffId, setDoctorId] = useState("");
  ...
  const getDoctorColor = (() => {
    ...
    let colorIndex = 0;
    return (staffId) => {
      if (!colorMap.has(staffId)) {
        colorMap.set(staffId, doctorColors[colorIndex % doctorColors.length]);
        colorIndex++;
      }
      return colorMap.get(staffId);
    };
  })();
  ...
  const fetchAppointments = async () => {
    try {
      const res = await fetch("http://localhost:5000/appointmentsstaff");
      const data = await res.json();
       console.log(data)
      const appointmentEvents = data.map((appt) => ({
        title: `${appt.staffId?.user?.firstName || "Médecin"} (${appt.staffId?.speciality || ""})`,
        start: new Date(appt.start),
        end: new Date(appt.end),
        allDay: false,
        staffId: appt.staffId?._id,
        id: appt._id,
      }));
  ...
  const filteredEvents = selectedDoctor
    ? events.filter((event) => event.staffId === selectedDoctor)
    : events;
...
console.log(availableDoctors)
      for (const doc of doctors) {
        try {
          const response = await fetch("http://localhost:5000/appointmentsstaff", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ staffId: doc._id, start, end }),
          });
  ...
  const handleManualAssignment = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/appointmentsstaff", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ staffId, start, end }),
      });

      const saved = await response.json();
      const doctor = doctors.find((doc) => doc._id === staffId);
  ...
  const eventStyleGetter = (event) => {
    const backgroundColor = getDoctorColor(event.staffId);
    return {
      style: {
        backgroundColor,
        color: "white",
