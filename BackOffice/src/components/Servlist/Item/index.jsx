...
import StaffService from 'services/ServiceMangerService';
import EditParamedicForm from '@pages/EditparamedicForm';
...
const Item = ({ type, data, onEdit, onDelete }) => {
  ...
  const handleDelete = async () => {
    try {
      await StaffService.deleteServiceManager(data._id);
      alert("paramedic deleted successfully!");
  ...
  const Common = () => (
    <Block>
      ...
      <div className="main">
        ...
        {type === 'paramedic' && role && <span className="role">{role}</span>}
  ...
  return (
    <AnimatePresence>
      ...
      {isEditPopupOpen && <EditParamedicForm staffData={data} closeForm={closeEditPopup} />}
    </AnimatePresence>
  );
};
