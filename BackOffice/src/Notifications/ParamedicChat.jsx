...
function ParamedicChat() {
  ...
  useEffect(() => {
    const fetchParamedics = async () => {
      try {
        ...
        console.error('Failed to fetch paramedics', error);
      }
    };
    fetchParamedics();
  }, []);
  ...
  const sendMessage = () => {
    if (!selectedParamedic) {
      alert('Please select a paramedic to send the message to.');
      return;
    }
    ...
    if (message.trim()) {
      ...
      socket.emit('send_message', {
        message,
        to: selectedParamedic.id, // identifiant unique du paramedic
      });
    ...
    <div style={{
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {paramedics.map((paramedic, i) => {
            ...
            return (
              <li
                key={paramedic.id || i}
                onClick={() => setSelectedParamedic(paramedic)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  padding: '10px',
                  borderRadius: '8px',
                  backgroundColor: selectedParamedic?.id === paramedic.id ? '#cce5ff' : '#e9ecef',
                  marginBottom: '10px',
                  gap: '10px',
                  cursor: 'pointer',
                }}
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', maxWidth: '100%' }}>
        <div style={{ padding: '20px', borderBottom: '1px solid #ddd', backgroundColor: '#fff' }}>
          <h2 style={{ margin: 0 }}>
            💬 Chat with: {selectedParamedic ? `${selectedParamedic.user?.firstName} ${selectedParamedic.user?.lastName}` : 'No one selected'}
          </h2>
        </div>
