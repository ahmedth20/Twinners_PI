...
function Login() {
  ...
  useEffect(() => {
    if (loginResult === "SUCCESS") {
      ...
      if (counter === 0) {
        ...
        // 👇 Ajoute la logique de redirection par rôle ici :
        if (tempAccount?.role) {
  ...
  const scanFace = async () => {
    ...
    const faceApiInterval = setInterval(async () => {
      ...
      const results = resizedDetections.map((d) => {
        ...
        if (bestMatch && bestMatch.distance < 0.2) {
          return bestMatch;
        }
        return null;
      });
  ...
  async function loadLabeledImages() {
    ...
    console.log("Loading labeled images for:", tempAccount);
    const descriptions = [];
    let img;
    ...
    const label = tempAccount.firstName || "Unknown"; // Use first name or fallback
    console.log("Creating LabeledFaceDescriptors with label:", label);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-[24px] max-w-[840px] mx-auto">
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-rose-700 sm:text-4xl">
          <span className="block">Profile picture doesn't match.</span>
        </h2>
